    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if conf < 80 and label in label_map:
                ...
                save_system_backup(student_id, student_name, timestamp)
...
# ----------------- EXTRA: Store attendance in system backup folder -----------------
SYSTEM_BACKUP_FOLDER = "system_attendance_backup"
os.makedirs(SYSTEM_BACKUP_FOLDER, exist_ok=True)

def save_system_backup(student_id, student_name, timestamp):
    backup_file = os.path.join(SYSTEM_BACKUP_FOLDER, f"backup_{datetime.now().strftime('%Y%m%d')}.csv")
    row = {"ID": student_id, "Name": student_name, "Time": timestamp}
